...
class MotorDriver(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('encoder_cpr', value=2480)
        if (self.get_parameter('encoder_cpr').value == 0):
            self.get_logger().warn("ENCODER CPR SET TO 0!!")


        self.declare_parameter('loop_rate', value=30)
        if (self.get_parameter('loop_rate').value == 0):
            self.get_logger().warn("LOOP RATE SET TO 0!!")
    ...
    def reset_arduino(self):
        try:
            self.get_logger().info(f"Resetting Arduino on port {self.serial_port}.")
            with serial.Serial(self.serial_port, 1200) as ser:
                ser.dtr = False
                time.sleep(0.1)
                ser.dtr = True
                time.sleep(1)  # Wait a second to ensure the reset is complete
            self.get_logger().info(f"Arduino on {self.serial_port} has been reset.")
